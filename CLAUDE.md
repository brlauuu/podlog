...
│   │   ├── src/app/api/            # API routes: search (search, grouped, mentions, speakers), feeds (CRUD, [id], [id]/poll, [id]/episodes, [id]/episodes/guids, preview), queue, audio, ask/coverage, episodes ([id], [id]/retry, [id]/speakers, [id]/speakers/merge, ingest, upload), docs/[slug], hardware, notifications (settings, test), meta-analysis (snapshot, refresh, coverage/missing-speakers), pipeline (ask, embed, embed-model-state, explore/status, health, queue/[episodeId]/retry), backups ([tier]/[filename], audio/[date], retention), prompts ([key], [key]/reset), version
