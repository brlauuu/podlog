...
def test_archive_emits_done_event(mock_session_cls, mock_update, mock_write, mock_emit):
    ...
    mock_emit.assert_called_once_with(db, episode)
...
def test_archive_done_event_total_excludes_pre_transcription_wait(
    mock_session_cls, mock_update, mock_write, mock_emit
):
    ...
    # Runtime helper computes total from stage durations only.
    from app.services.notification_runtime import _compute_total_processing_duration_secs
    assert _compute_total_processing_duration_secs(event) == 180.0
...
@patch("app.tasks.archive.update_episode")
@patch("app.tasks.archive.SessionLocal")
def test_archive_emitter_receives_episode_for_payload_build(
    mock_session_cls, mock_update, mock_write, mock_emit
):
    """Task should delegate payload construction to notification runtime helper."""
    db = MagicMock()
    mock_session_cls.return_value = db
    ...
    _, emitted_episode = mock_emit.call_args[0]
    assert emitted_episode is episode


@patch("app.tasks.helpers.emit_episode_failed_event")
def test_worker_emits_failed_event_on_terminal_failure(mock_emit):
    ...
    mock_emit.assert_called_once_with(db, episode, error_class="OOM", error_message="Out of memory")


@patch("app.tasks.helpers.emit_episode_failed_event")
def test_no_failed_event_on_retryable_failure(mock_emit):
...
def test_total_duration_calculation_uses_stage_durations_only():
    episode = MagicMock()
    episode.transcribe_duration_secs = 120.0
    episode.diarize_duration_secs = 60.0

    from app.services.notification_runtime import _compute_total_processing_duration_secs
    assert _compute_total_processing_duration_secs(episode) == 180.0

    episode.transcribe_duration_secs = None
    episode.diarize_duration_secs = None
    assert _compute_total_processing_duration_secs(episode) is None
