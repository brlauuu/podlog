...
**Changelog:**
- v1.8 — Hardening pass on C1/C2. `normalized_name` is now computed in TypeScript (`apps/web/src/lib/normalizeName.ts`, mirroring Python `inference_helpers.normalize_name`) and passed as a bind parameter into the cache upsert, so "Dr. Jane Smith" and "Jane Smith" collapse to a single cache row instead of two. Migration 014's backfill applies the same honorific-stripping regex (two nested `regexp_replace` passes — same honorific set as Python) and stores `last_seen_at = COALESCE(MAX(e.published_at), NOW())` for historical rows so recency reflects when a name was last confirmed, not when the migration ran. A new setting `feed_speaker_cache_recency_days` (default 365, 0 disables) filters `get_feed_speaker_cache_priors` so a long-ago user confirmation cannot outrank a recent correction when the host changes. Web unit tests for the rename route were rewritten to mock `pool.connect()` (matching the merge-route pattern) so the transactional upsert is actually exercised (issue #523).
- v1.7 — Added per-feed speaker name cache (C1 + C2). New `feed_speaker_cache` table maps `(feed_id, speaker_label, normalized_name) → (display_name, occurrence_count, last_seen_episode_id, last_seen_at)` and is populated **only** from explicit user renames in the web PUT `/api/episodes/:id/speakers` endpoint — inference output never writes here, so the cache cannot self-reinforce. Each rename runs inside the same transaction that upserts `speaker_names` and increments `occurrence_count` on conflict. At inference time, `get_feed_speaker_cache_priors` returns cache entries with `occurrence_count >= 2` (one-off confirmations are too often guest episodes or typos to be a reliable prior), sorted `occurrence_count DESC, last_seen_at DESC`. These are seeded as HIGH-confidence host candidates via a new `"feed_speaker_cache"` entry in `METADATA_SOURCES`, placed at the **top** of `extract_metadata_candidates` ordering — user confirmation is ground truth and outranks publisher-declared tags (`podcast:person`, `itunes:author`) and observed recurrence (`recurring_host`). Cohost demotion in `classify_candidates` now preserves the source's own confidence for `feed_speaker_cache` as well (parallel to `podcast_person_*`), so multi-host shows keep both speakers at HIGH. Alembic migration 014 creates the table and backfills from existing `speaker_names` rows with `confirmed_by_user=true`, grouping by `(feed_id, speaker_label, normalize(display_name))` so historical renames aggregate correctly (issue #523).
- v1.6 — Implemented the §4.2 recurring-host rule (A1). `get_recurring_host_name` queries the feed's last `recurring_host_window` (default 10) done episodes (ordered `published_at DESC, id DESC` for determinism) and counts `SPEAKER_00` display names from `speaker_names` rows that are either user-confirmed or HIGH-confidence inferred. The name that appears in at least `recurring_host_threshold` (default 0.8) of that window — with a hard floor of 3 occurrences — is seeded as a host candidate with source `recurring_host`. Emitted confidence is **MEDIUM** (not HIGH as §11 originally sketched) so the rule's own output rows cannot satisfy the HIGH filter on subsequent cycles — this blocks the self-reinforcement cascade where a wrong recurring inference would compound itself across future episodes. Every cycle must still consume a legitimate HIGH row (from `podcast:person`, `itunes:author`, feed-title match) or a user-confirmed row before the rule can fire. The candidate is inserted into `extract_metadata_candidates` after the publisher-declared tags (`podcast_person_*`, `itunes_author`) — which remain authoritative — and before `itunes_owner` / `episode_author`. Cohost demotion in `classify_candidates` preserves the source's own confidence for `recurring_host` and `podcast_person_*` (observed recurrence and publisher declarations are both strong enough for L-02 cases), while itunes sources still demote to LOW. On tied counts the most-recent episode's display casing wins via a newest-first secondary sort. Covers L-01 feeds where the host's name never appears in per-episode text. No migration required.
- v1.5 — Added Podcasting 2.0 `<podcast:person>` parsing (B2). Channel- and item-level tags are extracted from raw RSS XML (feedparser only keeps the last occurrence and strips either attributes or text depending on placement) and stored on `feeds.podcast_persons` / `episodes.podcast_persons` as JSONB. The namespace URI is matched under both `https://podcastindex.org/namespace/1.0` and the legacy `http://` form. Cast-group roles `host`/`cohost`/`co-host`/`guest host`/`presenter`/`interviewer` seed a HIGH-confidence host candidate; `guest`/`interviewee`/`subject` seed a HIGH-confidence guest. Empty/missing `role` defaults to `host` per spec. Production-crew roles (editor, narrator, etc.) are dropped. Item-level tags outrank channel-level tags via iteration order in `extract_metadata_candidates`, and the whole set outranks `itunes:author`/`itunes:owner` since publishers using `<podcast:person>` explicitly declared the role. Cohost demotion in `classify_candidates` preserves HIGH confidence for `podcast_person_*` sources (L-02 multi-host shows) while keeping the old LOW-demote behavior for itunes sources where the second slot is usually a business contact. Alembic migration 013 adds the two JSONB columns (issue #523).
...
| User rename populates cache | User renames SPEAKER_00 on ep1 and ep2 | `feed_speaker_cache` has entry with `occurrence_count = 2`; a subsequent episode inference seeds `host = that name, confidence = HIGH` |
| Cache prior outranks publisher-declared tags | Cache has "Alice" (count ≥ 2) and `<podcast:person>` declares "Bob" as host | `host = Alice, confidence = HIGH` (user confirmation is ground truth) |
