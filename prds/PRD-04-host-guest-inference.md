...
**Changelog:**
- v1.6 — Implemented the §4.2 recurring-host rule (A1). `get_recurring_host_name` queries the feed's last `recurring_host_window` (default 10) done episodes and counts `SPEAKER_00` display names from `speaker_names` rows that are either user-confirmed or HIGH-confidence inferred. The name that appears in at least `recurring_host_threshold` (default 0.8) of that window — with a hard floor of 3 — is seeded as a host candidate with source `recurring_host` and HIGH confidence. The LOW/MEDIUM filter on counted rows prevents self-reinforcement (a single wrong LOW guess cannot propagate into a HIGH signal on future episodes). The candidate is inserted into `extract_metadata_candidates` after the publisher-declared tags (`podcast_person_*`, `itunes_author`) — which remain authoritative — and before `itunes_owner` / `episode_author`. The cohost demotion in `classify_candidates` preserves HIGH confidence for `recurring_host` just like `podcast_person_*` (observed recurrence is strong enough for L-02 cases). Covers L-01 feeds where the host's name never appears in per-episode text. No migration required.
