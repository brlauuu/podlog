...
The Ask page lets users ask natural-language questions about their podcast transcripts. Answers are generated by a local LLM (Ollama) using Retrieval-Augmented Generation.

**How it works:**

1. The user's question is converted into an embedding vector using the same model used for chunk embeddings during ingestion.
2. The **top K** most similar transcript chunks are retrieved from the database by cosine similarity (`pgvector`). Only chunks above a minimum similarity threshold (0.3) are included.
3. The retrieved chunks are assembled into a prompt with a system instruction that tells the LLM to answer only from the provided excerpts and cite sources as `[Episode Title, MM:SS]`.
4. The LLM streams its response token by token via Server-Sent Events.

**Key parameters:**

| Parameter | Default | Location | Description |
|---|---|---|---|
| `TOP_K` | 8 | `rag.py` | Number of transcript chunks retrieved as context. Lower values (3-4) produce faster but potentially less complete answers. Higher values (12-16) give the LLM more material but increase response time. Values above ~25 risk exceeding the model's context window (4096 tokens for default models), causing truncated or degraded output. |
...
**Speaker attribution:** Retrieved chunks include speaker display names (from the speaker rename table) rather than raw labels, so the LLM can reference speakers by name in its answer.
