...
### Fixes
- The queue dashboard no longer loses sight of episodes mid-pipeline. Chunking and Embedding had no segment in the stage bar, so an episode vanished from it after Diarizing and reappeared at Inferring, with a grey badge in between; both now have their own segment and can be filtered like any other stage. Episodes that contain no speech were worse off — they appeared in no part of the dashboard at all, because every query behind it filtered them out. They now show up under Done with a "NO SPEECH" badge. Separately, an episode being chunked was reported with a status the database never uses, so the interface and the database disagreed about what that state was called. ([#968](https://github.com/brlauuu/podlog/issues/968))
