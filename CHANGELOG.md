...
### Internal
- The CI vulnerability audit stopped covering PyTorch when the CPU wheels landed, and said nothing about it. `pip-audit` cannot look up `torch==2.8.0+cpu` on PyPI, so it printed "could not be audited" and moved on — the reported finding count fell from 14 to 6, which reads like an improvement and was not one. The build-variant suffix is now stripped before the audit runs, so the version that is actually installed gets checked, and a new step fails the build if any package ever again carries a suffix that would make it unauditable. ([#977](https://github.com/brlauuu/podlog/issues/977))
