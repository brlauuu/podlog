...
### Internal
- Added unit tests for the `/api/backups` web proxy route — success, non-200 upstream surfaced with the fallback body, and network failure → 502 with the fallback body. ([#666](https://github.com/brlauuu/podlog/issues/666))
- Added unit tests for the `/api/prompts` proxy routes (GET list, PUT `[key]`, POST `[key]/reset`) — verifies upstream-status pass-through and URL-encoding of the key parameter. ([#667](https://github.com/brlauuu/podlog/issues/667))
- Added unit tests for the `/api/pipeline/explore/status` proxy route — running, not-running, and upstream-error shapes. ([#668](https://github.com/brlauuu/podlog/issues/668))
- Added page tests for `/feeds` covering the empty + populated list, the Add Feed POST flow, the per-row Poll call, and the Delete confirmation flow. ([#669](https://github.com/brlauuu/podlog/issues/669))
- Added page tests for `/podcasts` covering the populated-feeds branch, the "no sources yet" empty-state CTA, and the uploads-only branch. ([#670](https://github.com/brlauuu/podlog/issues/670))
- Added page tests for `/search/print` covering the missing-query 404, the empty-feeds 404, the populated print report (episode header, links, mention card, before/after context), and the no-display-name "Speaker" fallback. ([#671](https://github.com/brlauuu/podlog/issues/671))
- Added entry-point tests for `/meta-analysis` and `/docs` — confirms the client child renders, that `/docs` lists discovered `.md` files (with the "NN-" prefix stripped and Title Cased) and forwards the search index, and that the directory-read failure path falls back to an empty docs list. ([#672](https://github.com/brlauuu/podlog/issues/672))
- Lifted unit-test coverage on the web side for nine previously under-tested files — DarkModeToggle, SearchPagination, SearchResult, SearchResultsToolbar, FeedGroupCard, EpisodeMentionList, the Meta-Analysis InfoBlock and ChartCard, and `lib/docs-index.ts`. Every file is now ≥83% statement coverage. ([#673](https://github.com/brlauuu/podlog/issues/673))
- Added tests for `lib/formatFileSize.ts` covering every unit-range branch (B / KB / MB / GB) plus the negative-input edge case. ([#674](https://github.com/brlauuu/podlog/issues/674))
- Lifted pipeline unit-test coverage for the prompts service, prompts API router, and the spaCy load/unload helpers in `services/inference.py` — each file now sits at ≥98% statement coverage. ([#675](https://github.com/brlauuu/podlog/issues/675))
- `@types/node` patch-bumped to 20.19.41 (latest 20.x). We intentionally stay on the Node 20 line because the runtime in `apps/web/Dockerfile` (`node:20-alpine`) and `engines.node` (`>=20.9.0`) are both pinned to 20; jumping `@types/node` to 25 would expose APIs the runtime doesn't have. ([#677](https://github.com/brlauuu/podlog/issues/677))
- Bundled npm minor/patch bumps via `npm update` — `@tanstack/react-query` 5.100.10, `eslint-config-next` 16.2.6, `isomorphic-dompurify` 3.12.0, `lucide-react` 1.14.0, `next` 16.2.6, `postcss` 8.5.14, `react` / `react-dom` 19.2.6, `zod` 4.4.3 (and assorted dev-deps). All caret ranges already permitted these; lock-file only changes. ([#678](https://github.com/brlauuu/podlog/issues/678))
- New `scripts/check_docs_sync.py` is now part of the fast CI lane — it scans `CLAUDE.md` and `docs/development.md` for `apps/…` / `prds/…` / `docs/…` / `scripts/…` / `.github/…` path mentions and fails the build when any of them no longer exists on disk. Catches the staleness class the v2026-05-06 audit flagged before it can land on `main`. ([#679](https://github.com/brlauuu/podlog/issues/679))
- CLAUDE.md Operational Gotchas gains a short note on the ESLint 9 → 10 upstream block (`eslint-plugin-react@7.37.5` uses the removed `context.getFilename` API) so future audit re-discoveries of the same finding are recognized as duplicates of #494. Note self-deletes once the bundled plugin reaches ≥ 7.38.
- `apps/web/src/components/EpisodeChat.tsx` was 531 LOC; the Markdown/plain-text conversation export helpers move into `EpisodeChatExports.ts` and the assistant/user message bubble + citation handling move into `EpisodeChatMessage.tsx`. The main component now owns just the SSE streaming, model selection, and panel chrome. Pure code motion, no behavior changes. ([#665](https://github.com/brlauuu/podlog/issues/665))
- `apps/web/src/app/feeds/page.tsx` was 519 LOC; the two Add Feed dialog steps (URL + mode form, and the episode-selection list used by both selective-add and the issue-#487 "add more" flow) move into colocated subcomponents under `apps/web/src/app/feeds/_components/`, and the fetcher helpers + types move into `_lib/`. Pure code motion, no behavior change. ([#664](https://github.com/brlauuu/podlog/issues/664))
- `apps/web/src/components/RemoteInferenceSectionParts.tsx` was 645 LOC; split along its natural seams — pipeline-step descriptors and helpers into `RemoteInferencePipelineSteps.ts`, the step-card UI cluster (cards + estimates explainer + API-key warning dialog) into `RemoteInferencePipelineCards.tsx`, the Fireworks provider intro and API-key entry into `RemoteInferenceFireworksParts.tsx`, and the pyannote-cloud intro / API-key entry into `RemoteInferencePyannoteParts.tsx`. The original module is now a thin re-export barrel so existing imports keep working. ([#663](https://github.com/brlauuu/podlog/issues/663))
- `apps/pipeline/app/services/meta_analysis.py` was 565 LOC; split out the per-feed / per-episode / per-speaker / timeline / coverage builders into `meta_analysis_aggregations.py` so the orchestrator (stale helpers, `compute_snapshot`, `upsert_snapshot`, `recompute_and_store`) lives on its own. All previously-importable helpers are still importable from `meta_analysis` for back-compat with existing call sites and tests. ([#662](https://github.com/brlauuu/podlog/issues/662))
...
- `docs/development.md` refreshed — project-structure tree now lists the new `backups`, `prompts`, `explore` API routers and the new web `src/lib/` utilities; the Makefile section now documents `up-remote`/`down-remote`/`logs-remote`, `ollama-pull`, the `explore` family, the backup commands (`backup-now`, `backup-list`, `restore-db`, `restore-audio`), `backfill`, `version`, and `test-healthcheck`; prerequisites text clarifies that Docker is for the full stack, not just the database. ([#659](https://github.com/brlauuu/podlog/issues/659))
- RISKS-AND-GAPS RISK-03 reconciled with PRD-02 v1.3 — the queue-dashboard warm-up banner is documented as removed (warm-up is brief enough that a persistent UI element is no longer warranted); the `WARMING_UP` health state at `GET /api/health` is retained as the documented mitigation. ([#660](https://github.com/brlauuu/podlog/issues/660))
- Minor doc cleanups — README "Export" bullet now describes the `/search/print` view rather than a non-existent PDF generator, PRD-01 changelog gains a v1.7 entry noting the Fireworks RAG provider (#608), and the docs/hardware.md "Tested Machine" Docker/Compose versions are refreshed to the current installed versions. ([#661](https://github.com/brlauuu/podlog/issues/661))
...
### Fixes
- New "Under the hood: how speaker inference works" section in the [Speaker Management guide](docs/guide/06-speakers.md) — covers the pipeline stages, where each name candidate comes from with its confidence level, the host/guest classification rules, the run-based slot assignment logic (including why some speakers come up as Other), every tuning knob in `app.config.Settings`, and the known tradeoffs. Includes GitHub source links pinned to the relevant functions so the doc and code stay aligned. Final PR in the #703 series. ([#703](https://github.com/brlauuu/podlog/issues/703))
- Speaker inference now reads the first ≤ 300 s of transcript text as a fifth NER source ("transcript_intro"). Catches host / guest self-introductions like "Today I'm joined by Dror Poleg" that don't show up in the description — previously a guest whose name was only spoken on the air went uninferred. Capped at 300 s OR 150 segments, whichever comes first, to limit spaCy runtime and avoid casual-mention false positives later in the show. PR 4 of 5 for #703. ([#703](https://github.com/brlauuu/podlog/issues/703))
- Speaker inference: recurring-guest cache entries no longer pollute episodes the person isn't in. `feed_speaker_cache` rows for the SPEAKER_00 slot (recurring host) are still seeded as host candidates unconditionally; rows for SPEAKER_NN slots (recurring guests / producers) are now only seeded when this episode's title, description, or `<podcast:person>` tags actually mention the cached name. Substring match is on the normalized name, so honorifics in the cached entry don't block corroboration. PR 3 of 5 for #703. ([#703](https://github.com/brlauuu/podlog/issues/703))
- Speaker inference now fragments fully-short pyannote labels into per-run `role='other'` rows instead of guessing a name for them. A pyannote label is treated as a real speaker if it has at least one run whose duration is ≥ 15 s OR whose segment count is ≥ 20; otherwise each run becomes its own SPEAKER_NN slot marked as Other, so it shows up in the episode's "Other" speaker section ready to merge. Real speakers' brief interjections still stay with the parent label (run boundary = 2 s of silence with no intervening speaker). Fixes the long-standing "cold-open / skit voice gets named as the host" miss. PR 2 of 5 for #703. ([#703](https://github.com/brlauuu/podlog/issues/703))
...
- **Delete individual backups from Settings → Backups.** Each DB dump (daily/weekly/monthly) and each audio snapshot now has a Delete button. Clicking opens a confirmation modal showing what's about to be removed; on confirm, the file/dir is unlinked and the dashboard refreshes. Hardlinks across tiers are unaffected (only the named directory entry is removed). Today's audio snapshot is refused when the day's backup tick hasn't finished yet (mid-rsync guard) — retry once the next tick lands. Pipeline `/backups` mount is now read-write to support these endpoints; the only writes the pipeline performs there are the two DELETE handlers, both gated by tier/filename/date regex anchors and a path-traversal guard. ([#687](https://github.com/brlauuu/podlog/issues/687))
