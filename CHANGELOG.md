...
- Queue dashboard bulk-retry for episodes that failed because Fireworks rejected the upload. A new banner shows the eligible count, total audio minutes, and Fireworks cost estimate. The "Retry with chunking" button is disabled until chunked transcription is enabled in Settings. Catches both post-#600 (`FIREWORKS_UPLOAD_REJECTED`) and pre-#600 (`TRANSIENT_NETWORK` with the SSL `BAD_RECORD_MAC` signature) failures. ([#610](https://github.com/brlauuu/podlog/issues/610))
- RAG / Ask is now an independently toggleable remote-inference step in Settings. Pick local Ollama or Fireworks AI from a curated list of chat models (Qwen2.5 7B Instruct, Llama 3.1 70B Instruct, Qwen2.5 72B Instruct). The Remote Inference section also gets an explicit privacy notice spelling out that data for any step set to remote (audio, transcripts, queries, retrieved chunks, embedding inputs) leaves the local machine. Default for new installs and existing upgrades remains local. ([#608](https://github.com/brlauuu/podlog/issues/608))
...
- Diarization routing for chunked Fireworks transcription. When `fireworks_chunked_transcription_enabled` is on, the transcribe task forces `diarize=False` on Fireworks (per-chunk speaker IDs cannot be reconciled across chunks) and the diarize task runs the regular whole-file path governed by `diarization_provider` — pyannote.ai precision-2 cloud or local pyannote, the user's choice. ([#610](https://github.com/brlauuu/podlog/issues/610))
