...
### Fixes
- Manual upload retry no longer fails with `Invalid IDNA hostname` when the original filename had non-ASCII characters. The retry endpoint always queued a fresh `download` job, which fed the synthetic `local://<filename>` URL back into httpx — non-ASCII filenames tripped its IDNA encoding. `enqueue_episode_ingest` now detects manually-uploaded episodes (rows with a `local://` URL and an existing on-disk file) and starts them at `transcribe`, mirroring the original upload path. ([#650](https://github.com/brlauuu/podlog/issues/650))
