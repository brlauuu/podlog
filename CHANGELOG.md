...
### Minor changes
- `G` `S` now jumps to the Search page, and Settings moves to `G` `T`. That is the one chord whose meaning changed; every other one is unchanged. Press `?` on any page for the full list. ([#1061](https://github.com/brlauuu/podlog/issues/1061))
...
### Major changes
- The Telegram bot can now answer commands, so you can check on Podlog from your phone without being on the home network. Add the numeric Telegram user IDs that may talk to it under **Allowed user IDs** in Settings → Notifications → Telegram (in a private chat your Chat ID is your user ID; anyone else can ask the bot `/whoami`). Then send it `/queue` for what the pipeline is doing. The bot stays off until that list is filled in, refuses everyone not on it, and cannot change anything except adding a source with `/addfeed` (below). Notifications are unaffected. ([#1034](https://github.com/brlauuu/podlog/issues/1034))
- `/search <words>` in the Telegram bot searches your transcripts from anywhere: five hits per page with podcast, episode, speaker, timestamp and a snippet, plus a link that opens the episode at that moment when you are back on the home network. Same syntax as the search page, including `"exact phrase"`, `-exclude` and `speaker:Name`; add `p2` for the next page. ([#1035](https://github.com/brlauuu/podlog/issues/1035))
- `/transcript <episode>` in the Telegram bot sends an episode's transcript to the chat as a file, the same Markdown or plain-text export as the episode page's Export button. Give it an episode id or a few words from the title; with several matches you get a numbered list to pick from. The export is also served at `/api/episodes/<id>/transcript?format=txt|md` for scripts. ([#1037](https://github.com/brlauuu/podlog/issues/1037))
- `/ask <question>` in the Telegram bot puts Ask AI in your pocket: the bot replies "Thinking…" straight away, edits that message as the answer streams in, and finishes with the sources it used, linked to the moment in each episode. It uses whatever model and provider you configured in Settings, answers one question at a time, and keeps answering `/queue` and `/search` while it thinks. ([#1036](https://github.com/brlauuu/podlog/issues/1036))
- `/addfeed <full|test|selective> <rss url>` in the Telegram bot adds a podcast source from your phone. Full and test answer with one line, "Added `<title>`" or the reason it did not work; selective shows the five newest episodes and you reply with one number to add that episode. It is the bot's only command that changes anything, and it is undone with one click on the Feeds page. ([#1053](https://github.com/brlauuu/podlog/issues/1053))

### Minor changes
- A feed can now be made **selective** after the fact. A new **Make selective** button on test and full feeds stops automatic ingestion for good, keeps everything already ingested, and hands you the **Add episodes** picker for anything further; a paused feed is unpaused on the way, since selective feeds have nothing to pause. Until now the only way out of a chatty feed you had paused was to promote it back to full and take everything. ([#1045](https://github.com/brlauuu/podlog/issues/1045))
- Podlog now checks at startup whether your HuggingFace token works and whether the pyannote licence has been accepted for it, instead of letting the first episode finish without speakers an hour later. A failed check shows as a red notice on the queue page saying exactly what to fix, with the link, and as `Diarization: DEGRADED` in the health endpoint; it clears by itself once fixed. Skipped for cloud diarization, and an unreachable HuggingFace reports "could not verify" rather than an error. ([#1048](https://github.com/brlauuu/podlog/issues/1048))
- On a fresh install the queue page now says that the worker is downloading speech models and that new episodes will start when it finishes, instead of an empty state that looked broken while nothing in the UI mentioned the 5–15 minute first-run download. The banner reads the worker's warm-up state from the health endpoint and disappears on its own. ([#1047](https://github.com/brlauuu/podlog/issues/1047))

### Fixes
- The install docs now agree with each other and with the update script. The installation guide leads with the same path as the README (check out the newest tag, `make up-release`), both tell you to run `make ollama-pull` so the first Ask AI question does not fail with "Model not available", the first-run guide gains a day-one tour of the Settings tabs, and the loopback-only instruction in the Security model now uses `docker-compose.override.yml`, since editing `docker-compose.yml` left the working copy dirty and made `make update` refuse to run. ([#1046](https://github.com/brlauuu/podlog/issues/1046))
...
### Internal
- `nltk` bumped from 3.10.2 to 3.10.3 in the pipeline lock, clearing 16 of the 17 advisories the dependency audit had started reporting (the last one concerns parser APIs WhisperX never calls and is triaged); nltk is a transitive dependency of WhisperX used only for the alignment step's sentence splitter, and alignment was re-run on real audio to confirm. The audit job now ignores the advisories already triaged as unfixable-until-WhisperX-moves, listed in `apps/pipeline/audit-ignore.txt`, so a red audit once again means something new. ([#1033](https://github.com/brlauuu/podlog/issues/1033))
...
- New **Updating** page in the guide, covering what `make update` protects against at each step, the stable and edge channels, how to roll back, and the fact that published images are for 64-bit Intel/AMD machines only. The README now leads with installing a released version and keeps building from source as the development path. ([#937](https://github.com/brlauuu/podlog/issues/937))
